...
def main():
                ...
                while file2 is not None:
                    ...
                    for batch_start in range(starting_index, len(file1), batch_size):
                        ...
                        if batch_end <= len(file1):
                            ...
                            new_error, new_swh = func(batch_x, batch_y)
                            error_tensor = torch.cat([error_tensor, new_error])
                            swh_tensor = torch.cat([swh_tensor, new_swh])
                        # batches using 2 files
                            ...
                            swh_tensor = torch.cat([swh_tensor, new_swh])
                    del file1
                return error_tensor, swh_tensor
    ...
    error_tensor, swh_tensor = calculate_errors()

    #############################################
    #             SAVING RESULTS
    #############################################
    error_tensor = error_tensor.cpu()
    swh_tensor = swh_tensor.cpu()

    Path(f'{args.debugging_file}-values.pt').unlink(missing_ok=True)
    torch.save({'error': error_tensor, 'swh': swh_tensor}, f'{args.debugging_file}-values.pt')
